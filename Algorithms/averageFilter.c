...
void averageFilter(const size_t rows, const size_t cols, unsigned char* image,const size_t windowSize)
{
        ...
        for (int r = 0; r < (int)rows; ++r)
        {
            for (int c = 0; c < (int)cols; ++c)
            {
						...
						((col + guardSize) < (int)cols) )
				{
					...
					for(int element = 0; element < numPixels2Sort; element++)
					{
						averageSum += pixelValues[element];
					}
					average = averageSum / numPixels2Sort;

					filteredImage[row*cols + c] = average;
